...
class NBAProjectionSystem:
    ...
    def fetch_team_stats(self):
        ...
        try:
            ...
            if datetime.now().month >= 10:  # After October, use next year's season
            ...
            print("   Will use default team stats")
            # Create default stats for common teams
            default_teams = ['ATL', 'BOS', 'BKN', 'CHA', 'CHI', 'CLE', 'DAL', 'DEN', 'DET', 'GSW',
    ...
    def fetch_game_logs_with_retry(self):
        """Fetch game logs with retry logic and correct date range"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == 0:
                    # First try with season parameter
                    ...
                    # Try just last 10 days to reduce data
                    recent_start = end_date - timedelta(days=10)
                    date_from_small = recent_start.strftime('%m/%d/%Y')
                    logs = playergamelogs.PlayerGameLogs(
                        date_from_nullable=date_from_small,
                        date_to_nullable=date_to,
                        timeout=60
                    )
        ...
        print("❌ All attempts to fetch game logs failed")
        return False
    ...
    def run(self, output_file: str = None):
        ...
        if not self.fetch_game_logs_with_retry():
            print("❌ Failed to fetch game logs - cannot generate projections")
            print("   This usually means:")
            print("   1. NBA API is down or rate limited")
            print("   2. There are no games in the date range")
