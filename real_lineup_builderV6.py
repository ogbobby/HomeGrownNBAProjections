...
class EnhancedProjectionsNBAData:
    ...
    def calculate_enhanced_projection(self, player, recent_games, position, team, dk_salary_info):
        ...
        try:
            ...
            # Use ACTUAL DK salary - MOVED UP BEFORE UPSIDE CALCULATION
            salary = dk_salary_info['salary']
            
            # Calculate ceiling projection (best recent game)
            ceiling_projection = self.calculate_ceiling_projection(recent_games, projected_minutes)
            
            # Calculate upside score - NOW salary IS DEFINED
            upside_score = self.calculate_upside_score(projection, ceiling_projection, salary, usage_rate, volatility_score, matchup_difficulty)
    ...
    def calculate_ceiling_projection(self, recent_games, projected_minutes):
        """Calculate ceiling projection based on best recent performance"""
        try:
            ...
            if best_game_minutes > 0 and projected_minutes > 0:
                ...
                # Apply a reasonable cap (don't assume they'll exceed their best game by too much)
                ceiling_projection = min(ceiling_projection * 1.1, best_game_score * 1.25)
            ...
            # Fallback: use 25% above projection as ceiling
            return 0

    def calculate_upside_score(self, projection, ceiling_projection, salary, usage_rate, volatility_score, matchup_difficulty):
        """Calculate upside score (0-100) for identifying high-upside players"""
        try:
            score = 0
            
            # Ceiling vs Projection gap (40% of score)
            if projection > 0 and ceiling_projection > projection:
                ceiling_gap = (ceiling_projection - projection) / projection
                ceiling_component = min(40, ceiling_gap * 100)
                score += ceiling_component
            
            # Salary-based value (20% of score) - cheaper players have more upside
            if salary > 0:  # Safety check
            ...
            # Volatility (15% of score) - volatile players have more upside potential
            volatility_component = min(15, (volatility_score - 0.5) * 20)
...
class EnhancedProjectionsNBAOptimizer:
    ...
    def build_lineups(self, strategy='balanced', num_lineups=1):
        ...
        print(f"\n🚀 ENHANCED PROJECTIONS NBA DFS LINEUP BUILDER - {strategy.upper()} STRATEGY")
...
if __name__ == "__main__":
    ...
    print("With multiple lineup strategies: balanced, high-upside, stars & scrubs, high-floor")
    print()
    ...
    if choice == "2":
        ...
        if max_points_lineup:
            ...
            optimizer.display_lineup(max_points_lineup)
            success = True
        else:
            success = False
